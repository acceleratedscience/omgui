...
class GUIFileSystemService:
    ...
    def delete_file(self, path_absolute=""):
        ...
        """
        trash_dir = self.ctx.workspace_path() / ".trash"
    ...
    def get_files(self, path=""):
        ...
        for category, items in level.items():
            ...
            for index, filename in enumerate(items):
                ...
                items[index] = self._compile_filedir_obj(file_path)

        #
        #

        # Attach workspace name
        workspace_name = self.ctx.workspace
        ...
        if (
            level["_meta"]["empty"]
            and not level["filesHidden"]
            and not level["dirsHidden"]
        ):
    ...
    def get_file(self, path="", query=None):
        ...
        file_obj = self._compile_filedir_obj(path)
        file_type = file_obj.get("_meta", {}).get("fileType")

        # Attach data for files
        if file_type and file_type != "dir":
            file_obj = self._attach_file_data(file_obj, query)

        return file_obj

    def _compile_filedir_obj(self, path):
        """
        ...
        if os.path.isfile(path_absolute):
            ...
            ext2 = self._get_file_ext2(filename)
            file_type = self._get_file_type(ext, ext2)
            return {
                "_meta": {
                    "fileType": file_type,
                    "ext": ext,
    ...
    def _attach_file_data(self, file_obj, query=None):
        """
            ...
            if molset:
                ...
                if ext == "json":
                    cache_id = create_molset_cache_file(
                        self.ctx, path_absolute=path_absolute
                    )

                # All other cases, write file from memory.
                else:
                    cache_id = create_molset_cache_file(self.ctx, molset=molset)
    ...
    def _get_file_ext2(self, filename):
        """
    ...
    def _get_file_type(self, ext, ext2):
        """
