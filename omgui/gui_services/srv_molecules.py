...
def add_mol_to_mws(
    ...
    if smol:
        ...
        if enrich:
            _, identifier = get_best_available_identifier(smol)
            smol_enriched = find_smol(identifier, enrich=True)
            if smol_enriched:
                smol = merge_smols(smol, smol_enriched)

    # Identifier -> enrich by default
    else:
        smol = find_smol(identifier, enrich=enrich)
...
def remove_mol_from_mws(identifier: str = None, smol: dict = None, silent=False):
    """
